...
import { createFont, getVariableValue, isWeb } from "tamagui";
import { FillInFont } from "@tamagui/core";
...
const defaultSizes = isWeb ? webSizes : nativeSizes;

// line height: native ~125% per iOS HIG, web ~160%
const defaultLineHeight = (size: number) =>
  Math.round(isWeb ? size * 1.05 + 8 : size * 1.25);
...
const headingLineHeight = (size: number) =>
  Math.round(isWeb ? size * 1.12 + 5 : size * 1.2);
